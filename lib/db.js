...
export async function createEvent(fields) {
  ...
  const result = await db
    .prepare(`INSERT INTO events (title, artists, date, venue, location, description, type, status, image, highlight, ticketUrl, instagramUrl)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(
      fields.title || '',
      artists,
...
export async function updateEvent(id, fields) {
  ...
  await db
    .prepare(`UPDATE events SET title=?, artists=?, date=?, venue=?, location=?, description=?, type=?, status=?, image=?, highlight=?, ticketUrl=?, instagramUrl=?
              WHERE id=?`)
    .bind(
      ...
      fields.instagramUrl !== undefined ? fields.instagramUrl : (existing.instagramUrl || ''),
      id,
    )
    .run()
...
// ── Latin Legacy Tour Subscribers ────────────────────────────────────────────

export async function addLatinLegacySubscriber(email, name = '') {
  const db = getDB()
  try {
    await db
      .prepare('INSERT INTO latin_legacy_subscribers (email, name) VALUES (?, ?)')
...
export async function getLatinLegacySubscribers() {
  const { results } = await getDB()
    .prepare('SELECT * FROM latin_legacy_subscribers ORDER BY subscribed_at DESC')
    .all()
  return results
}
...
export async function setAdminConfig(key, value) {
  await getDB()
    .prepare(`INSERT INTO admin_config (key, value, updated_at) VALUES (?, ?, unixepoch())
