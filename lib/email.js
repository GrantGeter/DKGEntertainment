...
// Lazy-initialized so the constructor never runs at Next.js build time
function getResend() {
  return new Resend(process.env.RESEND_API_KEY)
}

function FROM() { return process.env.DKG_FROM_EMAIL || 'DKG Entertainment <onboarding@resend.dev>' }
function TEAM_EMAIL() { return process.env.DKG_TEAM_EMAIL || 'info@dkgentertainment.com' }
...
export async function sendContactAutoReply({ name, email }) {
  ...
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <hr class="divider">
      <div class="footer">DKG Entertainment · <a href="${siteUrl}">${siteUrl.replace('https://', '')}</a></div>
    `),
  })
}
...
export async function sendSubscribeConfirmation({ email, name, token }) {
  ...
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <hr class="divider">
      <div class="footer">
        Don't want emails? <a href="${siteUrl}/unsubscribe?token=${token}">Unsubscribe</a>
        &nbsp;·&nbsp; DKG Entertainment
      </div>
    `),
  })
    ...
    try {
      const r = await resend.emails.send({
        ...
        html: wrap(`
          ...
          <br>
          <a class="btn" href="${siteUrl}/news/${post.slug}">Read More</a>
          <hr class="divider">
          <div class="footer">
            You're receiving this because you subscribed to DKG Entertainment updates.
            <br><a href="${siteUrl}/unsubscribe?token=${sub.unsubscribe_token}">Unsubscribe</a>
          </div>
        `),
      })
    ...
    try {
      await resend.emails.send({
        ...
        html: wrap(`
          ...
          ${event.ticketUrl ? `<br><a class="btn" href="${event.ticketUrl}">Get Tickets</a>` : `<br><a class="btn" href="${siteUrl}/events">View Events</a>`}
          <hr class="divider">
          <div class="footer">
            You're receiving this because you subscribed to DKG Entertainment updates.
            <br><a href="${siteUrl}/unsubscribe?token=${sub.unsubscribe_token}">Unsubscribe</a>
...
export async function sendUnsubscribeConfirmation({ email }) {
  ...
  return getResend().emails.send({
    ...
    html: wrap(`
      ...
      <p class="value">Changed your mind? <a href="${siteUrl}">Visit our site</a> to sign up again.</p>
      <hr class="divider">
